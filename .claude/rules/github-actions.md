...
cargo-dist is pinned in `.mise.toml`, so `dist` inside the repo is the version
the workspace declares. Homebrew's `cargo-dist` formula floats and its bin
directory precedes mise's on a normal macOS PATH, so a bare `dist` can easily be
a different version — measured, 0.32.0 against a declared 0.30.3. The version
