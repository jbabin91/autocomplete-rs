...
| --------------- | --------------------------------------------------- | --------------------------------- |
| Static Analysis | rustfmt, clippy, dprint, rumdl, actionlint, dist pin parity, plus `cog check` on PRs | `.mise.toml` `check`; the cog step lives in `ci.yml` |
...
cargo-dist is pinned in `.mise.toml`, which is what lets each repo get its own
version — this one declares 0.30.3 while linesmith declares 0.31.0, so no single
global install serves both. The pin does not win on PATH by itself: a Homebrew
`cargo-dist` shadows it, because `/opt/homebrew/bin` precedes mise's tool
directories in a normal macOS shell — measured, a bare `dist` was 0.32.0 against
a declared 0.30.3 even inside `mise run`. Reach the pinned one as
`mise exec -- dist`. CI is unaffected: no CI job runs dist, and `release.yml`
fetches it by curl.

Nothing here checks that version, because dist enforces it itself — running a
mismatched binary fails with `You're running dist 0.32.0, but
'cargo-dist-version = 0.30.3' is set in your Cargo.toml`. Regenerating from the
wrong version's template is therefore not a reachable state, and a check for it
would only restate the tool's own guard.
...
5. On a cargo-dist upgrade, `dist init` updates `cargo-dist-version`; bump the
   `.mise.toml` pin to match, or dist refuses to run
